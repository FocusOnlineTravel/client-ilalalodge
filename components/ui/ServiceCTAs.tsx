...
export default function ServiceCTAs({ theme = 'light', align = 'center', className = '' }: Props) {
  ...
  const base =
    'inline-flex items-center gap-2 px-5 pt-2 pb-1.5 rounded-full text-sm font-semibold uppercase tracking-wide transition-all duration-200';
  ...
  return (
    <div className={`flex flex-wrap gap-3 ${justify} ${className}`}>
      ...
      <a href={`mailto:${EMAIL}`} className={email} aria-label={`Email ${EMAIL}`}>
        <Mail className="w-4 h-4" strokeWidth={1.75} />
        <span>Email Us</span>
        <span className="opacity-60 text-[11px] font-normal normal-case">(office hours only)</span>
      </a>
    </div>
  );
}
