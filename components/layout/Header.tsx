...
export default function Header() {
  ...
  return (
    ...
    >
      ...
      {/* Burger Menu */}
      <div
        className={`fixed inset-0 bg-brand-forest/98 backdrop-blur-md transition-transform duration-300 overflow-y-auto ${
          isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        <div className="min-h-full px-8 py-24 lg:px-16">
            ...
            <nav className="flex flex-col gap-4">
              {navLinks.map((link) => (
                <Link
                  ...
                  className="text-white hover:text-brand-gold transition-colors duration-200 text-lg font-medium border-b border-white/20 pb-2"
                >
                  {link.label}
                </Link>
              ))}
