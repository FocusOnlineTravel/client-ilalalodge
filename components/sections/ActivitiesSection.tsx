...
const getActivityIcon = (label: string) => {
  const iconClass = "h-8 w-8 text-brand-gold";
...
export default function ActivitiesSection({ data }: Props) {
  return (
    <section className="py-16 lg:py-24 bg-brand-forest text-white" id="activities">
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6 lg:gap-8 mb-12">
          {data.activities_items.map((activity, index) => (
            <div
              key={index}
              className="flex flex-col items-center text-center gap-3 p-4 bg-white/5 hover:bg-white/10 transition-colors"
            >
              <div className="bg-brand-daisy/10 p-4">
                {getActivityIcon(activity.activity_label)}
              </div>
              <span className="text-sm font-medium">{activity.activity_label}</span>
            </div>
          ))}
        </div>
